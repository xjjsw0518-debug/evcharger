...
The SE Asia EV charging accessories market is on the cusp of an explosion — now is the window to enter. The key is choosing the right entry point (products + country + channel), starting small with test batches, and scaling gradually. youpei auto is ready to be your reliable China supplier partner for entering this market. Wholesale inquiries welcome.`,
  ...
  // 12. 带视频的示例文章 - EV充电产品演示视频
  {
    id: 'blog-012',
    title: {
      zh: 'EV充电产品实机演示：便携式充电桩安装与使用全流程',
      en: 'EV Charging Product Demo: Portable EV Charger Installation & Usage Walkthrough',
    },
    category: 'product-guide',
    coverImage: 'https://picsum.photos/seed/ev-charging-demo-video/600/400',
    author: 'youpei auto Technical Team',
    summary: {
      zh: '通过视频演示，直观展示便携式交流充电桩的开箱检查、插座安装、车辆连接、充电启动与停止的完整流程，以及常见问题排查。',
      en: 'Video demonstration of the complete portable AC EV charger workflow — unboxing, socket installation, vehicle connection, charging start/stop, and common troubleshooting.',
    },
    content: {
      zh: `## 视频演示说明

本文附带产品演示视频，建议先观看视频再阅读文字说明。视频中展示了便携式交流充电桩（7kW/32A 可调电流款）的完整使用流程。

## 一、开箱检查

收到产品后，请第一时间检查以下内容：

1. **主机外观**：检查外壳是否有划痕、变形，显示屏是否完好
2. **配件清单**：确认包含主机、充电枪、电源线、说明书、保修卡
3. **线缆检查**：检查线缆是否有破损、接头是否牢固
4. **插头规格**：确认插头类型与您的插座匹配（国标16A/32A）

## 二、安装前准备

### 电源要求
- 16A 模式：普通家用空调插座即可
- 32A 模式：需要专用 32A 插座，建议由电工安装
- 插座必须有可靠接地
- 线路载流量需满足要求（32A 至少 4mm² 铜线）
...
1. 将电源插头插入插座，听到"咔嗒"声表示插紧
2. 显示屏亮起，显示当前电压和待机状态
3. 按下电流选择按钮，选择合适的充电电流（8A/10A/16A/32A）
4. 打开车辆充电口盖，将充电枪插入车辆充电口
5. 听到车辆"滴滴"提示音，表示连接成功
6. 充电桩自动开始充电，显示屏显示实时电流、电压、功率和已充电量
...
- **显示屏数据**：实时电压、电流、功率、温度
- **线缆温度**：正常充电时线缆微温属正常，烫手需立即停止
- **车辆状态**：车辆仪表或APP显示的充电状态
- **异常报警**：如出现故障码，参考说明书排查

## 五、停止与收纳

1. 按下充电桩上的停止按钮，或在车辆上停止充电
2. 等待充电枪解锁（通常有30秒延时）
3. 拔出充电枪，盖好车辆充电口盖
4. 拔出电源插头
5. 整理线缆，避免弯折过度
...
便携式充电桩使用简单，但正确的安装和使用习惯直接关系到充电安全和设备寿命。建议首次使用时仔细阅读说明书，并观看本演示视频。youpei auto 提供全系列便携式充电桩产品，支持 8A-32A 可调电流、LCD 显示屏、多重安全保护。批发客户欢迎咨询。`,
      en: `## Video Demo

This article includes a product demonstration video. We recommend watching the video first, then reading the text description. The video shows the complete workflow of a portable AC EV charger (7kW/32A adjustable current model).

## 1. Unboxing & Inspection

Upon receiving the product, immediately check:

1. **Unit appearance**: Check for scratches, dents, and display integrity
2. **Accessory list**: Verify main unit, charging gun, power cable, manual, warranty card
3. **Cable inspection**: Check for cable damage and secure connectors
4. **Plug type**: Confirm the plug matches your socket (China 16A/32A)

## 2. Pre-Installation Preparation

### Power Requirements
- 16A mode: Standard home air conditioner socket
- 32A mode: Dedicated 32A socket, professional electrician installation recommended
- Socket must have reliable grounding
- Wiring must meet current requirements (32A requires at least 4mm² copper wire)

### Environment Requirements
- Indoor or sheltered outdoor use
- Avoid water accumulation and direct sunlight
- Well-ventilated for heat dissipation
- Away from flammable/explosive materials

## 3. Connection & Startup

The video demonstrates these steps in detail:

1. Plug the power connector into the socket — click sound confirms secure connection
2. Display lights up, showing current voltage and standby status
3. Press current selection button to choose charging current (8A/10A/16A/32A)
4. Open vehicle charging port cover, insert charging gun
5. Vehicle beeps to confirm successful connection
6. Charger starts automatically, display shows real-time current, voltage, power, and charged amount

## 4. Monitoring During Charging

During charging, monitor:

- **Display data**: Real-time voltage, current, power, temperature
- **Cable temperature**: Slight warmth is normal — if hot to touch, stop immediately
- **Vehicle status**: Charging status on vehicle dashboard or app
- **Error alarms**: If fault code appears, consult manual

## 5. Stop & Store

1. Press stop button on charger, or stop charging from vehicle
2. Wait for charging gun to unlock (usually 30-second delay)
3. Remove charging gun, close vehicle charging port cover
4. Unplug power connector
5. Coil cable neatly, avoid excessive bending
6. Store in dry, well-ventilated area

## 6. Common Troubleshooting

### Q1: No charging after plugging in
- Check if socket has power
- Confirm vehicle charging port is fully inserted
- Check if vehicle has scheduled charging enabled
- Try switching current level

### Q2: Charging stops mid-way
- Check if socket is loose or overheating
- Confirm vehicle has charging limit setting
- Check for fault code display
- Try reducing charging current

### Q3: Cable gets very hot
- Stop charging immediately
- Check socket for poor contact
- Confirm wiring current capacity is sufficient
- Use lower charging current

## Summary

Portable EV chargers are simple to use, but proper installation and usage habits directly affect charging safety and equipment lifespan. We recommend reading the manual carefully and watching this demo video before first use. youpei auto offers a full range of portable EV chargers with 8A-32A adjustable current, LCD display, and multiple safety protections. Wholesale inquiries welcome.`,
    },
